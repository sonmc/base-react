...
const SidebarContent = (props) => {
    ...
    useEffect(() => {
        let group = currentUser.group_ids;
        if (group) {
            group = JSON.parse(group);
            let data = navData.filter((x) => {
                const routerGroup = JSON.parse(x.group);
                const isIncluded = group.every((item) => routerGroup.includes(item));
                return isIncluded;
            });
            setNav(data);
        }
                    ...
                    <React.Fragment key={key}>
                        {item['isHeader'] ? (
                            ...
                            <li className="nav-item">
                                <Link onClick={item.click} className={location.pathname == item.link ? 'nav-link menu-link active' : 'nav-link menu-link'} to={item.link ? item.link : '/#'} data-bs-toggle="collapse">
                            ...
                            <li className="nav-item">
                                <Link className={location.pathname == item.link ? 'nav-link menu-link active' : 'nav-link menu-link'} to={item.link ? item.link : '/#'}>
